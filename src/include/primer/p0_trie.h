...
class TrieNode {
 public:
   ...
   */
  TrieNode(TrieNode &&other_trie_node) noexcept : children_(std::move(other_trie_node.children_)) {
  ...
  std::unique_ptr<TrieNode> *InsertChildNode(char key_char, std::unique_ptr<TrieNode> &&child) {
    if ((key_char == child->key_char_) && (children_.count(key_char) == 0)) {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    while (ch != key.end()) {
      ...
      auto tmp = pre->get();
      auto ans = tmp->HasChild(*op);
      if (!ans) {
        pre = tmp->InsertChildNode(*op, std::make_unique<TrieNode>(*op));
      } else {
        pre = tmp->GetChildNode(*op);
      }
    }
    ch--;
    ...
    if (end_node != nullptr) {
      auto new_node = new TrieNodeWithValue(std::move(*(end_node->get())), value);
      end_node->reset(new_node);
      latch_.WUnlock();
      return true;
    }
    pre = pre->get()->InsertChildNode(*ch, std::make_unique<TrieNode>(*ch));
    auto new_node = new TrieNodeWithValue(std::move(*(pre->get())), value);
    pre->reset(new_node);
    latch_.WUnlock();
    return true;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    Removehelper(cur, key, i, ans, removeable);
    latch_.WUnlock();
    return ans;
  }
  void Removehelper(std::unique_ptr<TrieNode> *cur, const std::string &key, int &i, bool &ans, bool &removeable) {
    if (i == static_cast<int>(key.size()) && !cur->get()->HasChildren()) {
      removeable = true;
      ans = true;
      return;
    }
    ...
    auto next = cur->get()->GetChildNode(key[i]);
    if (next == nullptr) {
      removeable = false;
      ans = false;
      return;
    ...
    int j = i + 1;
    Removehelper(next, key, j, ans, removeable);
    if (removeable) {
      cur->get()->RemoveChildNode(key[i]);
    }

    removeable = !(cur->get()->HasChildren());
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    for (int i = 0; i < n; i++) {
      temp = (temp->get())->GetChildNode(key[i]);
      if (temp == nullptr) {
        *success = false;
        latch_.RUnlock();
        return {};
    ...
    auto *casted_node = dynamic_cast<TrieNodeWithValue<T> *>(temp->get());
