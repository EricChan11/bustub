...
void ExtendibleHashTable<K, V>::Insert(const K &key, const V &value) {
  ...
  while (dir_[IndexOf(key)]->IsFull()) {
    int local = dir_[IndexOf(key)]->GetDepth();
    auto index = IndexOf(key);
    auto target = dir_[index];
    ...
    int mask = 1 << local;
    auto b0 = std::make_shared<Bucket>(bucket_size_, local + 1);
    auto b1 = std::make_shared<Bucket>(bucket_size_, local + 1);
    num_buckets_++;
    for (const auto &[K1, V1] : target->GetItems()) {
      if ((mask & std::hash<K>()(K1)) == 0) {
        b0->Insert(K1, V1);
      } else {
        b1->Insert(K1, V1);
      }
